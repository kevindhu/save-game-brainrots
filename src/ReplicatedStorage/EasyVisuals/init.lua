...
function Effect.new<T...>(
	...
	if customColor then
		assert(
			typeof(customColor) == "ColorSequence" or typeof(customColor) == "Color3",
			"customColor is not a ColorSequence or Color3"
		)
	end
	if customTransparency then
		assert(
			typeof(customTransparency) == "NumberSequence" or typeof(customTransparency) == "number",
			"customTransparency is not a NumberSequence or number"
		)
	end
	...
	local function RecursiveAncestryChanged(Object: Instance)
		...
		table.insert(
			self.Connections,
			Object:GetPropertyChangedSignal(IsVisibleOrEnabled):Connect(function()
