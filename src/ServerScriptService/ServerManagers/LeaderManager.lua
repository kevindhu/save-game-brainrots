...
local Leader = require(game.ServerScriptService.Objects.Leader)
...
function LeaderManager:sendUserStats(itemClass)
	...
	for name, user in pairs(ServerMod.userManager:getAllUsers()) do
		if not user.initialized then
			continue
		end
		self:sendTotalToDatastore(user.statManager, itemClass)
	end
end
