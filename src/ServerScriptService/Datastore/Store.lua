...
function Store.new(user)
	...
	-- Can only not save in studio. Have to always save in production.
...
function Store:initProfile()
	...
	local profile = ServerMod.serverStoreManager:getPlayerProfile(
		user,
		profileKey,
		profileSettings.addUserId,
		profileSettings.viewOnly
	)
...
function Store:editProfile(profileKey)
	...
	local versionQuery = ServerMod.serverStoreManager:getVersionQuery(profileKey, minDate, maxDate)
...
function Store:release()
	...
	ServerMod.serverStoreManager:releaseProfile(profile)
end
