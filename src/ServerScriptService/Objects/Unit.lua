...
local UnitInfo = require(game.ReplicatedStorage.Data.UnitInfo)
...
function Unit:init()
	...
	local petData = self.user.saveManager.currWaveMod["petData"]
...
function Unit:die(totalDelay)
	...
	routine(function()
		...
		self.user.saveManager:killUnitFromWave(waveMod, self)
	end)
end
...
function Unit:captureSavedPet()
	...
	self.user.saveManager:failWaveMod(waveMod, self)

	routine(function()
		wait(5 / self.user.speedManager:getSpeed())
...
function Unit:getGoalFrame()
	...
	if actionClass == "WalkToSavePart" then
		return self.user.plotManager.saveModel.BasePart.CFrame
	else
...
function Unit:checkSafeZone()
	local safeZone = self.user.plotManager.safeZone
...
function Unit:calculateNewPos(currFrame, goalFrame, timeRatio)
	...
	local travelSpeed = self.baseMoveSpeed * timeRatio * self.user.speedManager:getSpeed()
