...
local MutationInfo = require(game.ReplicatedStorage.Data.MutationInfo)
...
function PetSpot:getTotalCoinsPerSecond()
	...
	-- local rebirthManager = self.user.rebirthManager
	-- coinsPerSecond = coinsPerSecond * rebirthManager.rebirthCoinsMultiplier

	-- local indexManager = self.user.indexManager
	-- coinsPerSecond = coinsPerSecond * indexManager.coinsMultiplier

	if self.user.shopManager:checkOwnsGamepass("2xCoins") then
...
function PetSpot:initBuyModel()
	local plotModel = self.user.plotManager.model
...
function PetSpot:tickAttack(timeRatio)
	...
	local attackSpeedRatio = self.attackSpeedRatio * self.user.speedManager:getSpeed()
	...
	-- gamepass multiplier
	if self.user.shopManager:checkOwnsGamepass("2xDamage") then
		damage = damage * 2
	end
...
function PetSpot:tryLevelUp()
	...
	if petData["level"] >= maxLevel then
		self.user.notifyManager:notifyError("Already max level!")
		return
	end
