...
function ToolManager.new(user, data)
	local u = {}
	u.user = user
	u.data = data
...
function ToolManager:tryBuyTool(data)
	...
	if coinsCount < price then
		self.user.notifyManager:notifyError("You don't have enough coins to buy this tool")
		return
	end

	if self:checkToolOwned(toolClass) then
		self.user.notifyManager:notifyError("You already own this tool")
		return
	end

	self.user.notifyManager:notifySuccess("You bought the " .. toolStats["alias"] .. "!")
...
function ToolManager:tryPlaceRelicAtPetSpot(data)
	...
	local petSpot = self.user.petManager.petSpots[petSpotName]
	if not petSpot then
		warn("NO PET SPOT TO PLACE PET AT: ", petSpotName)
		return
	end
	if not petSpot.unlocked then
		self.user.notifyManager:notifyError("This platform is not unlocked")
		return
	end
	if not petSpot.petData then
		self.user.notifyManager:notifyError("This platform is not occupied")
		return
	end
	if len(petSpot.petData["relicMods"]) >= 1 then
		self.user.notifyManager:notifyError("Brainrot already has a relic")
		return
	end
...
function ToolManager:tryPlacePetAtPetSpot(data)
	...
	if petSpot.petData then
		self.user.notifyManager:notifyError("This platform is already occupied")
		return
	end
