...
local ItemInfo = require(game.ReplicatedStorage.Data.ItemInfo)
...
function StashManager:generatePetData(data)
	...
	self.user.petManager:fillPetDataWithDefaults(itemData)
...
function StashManager:generateRelicData(data)
	...
	itemData["critChance"] = relicStats["critChance"] or 0
...
function StashManager:updateItemCount(data)
	...
	if count > 0 then
		if Common.listContains({ "Gems", "Coins" }, itemName) then
			self.user.statManager:incrementStatMod("Total" .. itemName, count)
...
function StashManager:removeItemMod(data)
	...
	local toolMod = self.user.toolManager.stashToolMods[itemName]
	if toolMod then
		toolMod:destroy()
	end
...
function StashManager:trySellAllToolItems(data)
	...
	if sellTotalPrice == 0 then
		self.user.notifyManager:notifyError("No brainrots to sell")
		return
	end
...
function StashManager:trySellItem(data)
	...
	if itemMod["favorited"] then
		self.user.notifyManager:notifyError("Cannot sell favorite brainrots")
		return
	end
...
function StashManager:tryUseItem(data)
	...
	if itemStats["tabGroup"] == "Boosts" then
		self.user.boostManager:addBoostMod(itemClass)
	end

	self.user.notifyManager:notifySuccess("Used " .. itemMod["alias"])
