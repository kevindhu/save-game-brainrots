...
local WaveInfo = require(game.ReplicatedStorage.Data.WaveInfo)

local SaveManager = {}
SaveManager.__index = SaveManager

function SaveManager.new(user, data)
	local u = {}
	u.user = user
	u.data = data
...
function SaveManager:init()
	...
	routine(function()
		...
		if self.user.tutManager.completedTutMods["CompleteTutorial"] then
			self:tryTogglePlay({
				newBool = true,
			})
		end
...
function SaveManager:startNewWaveMod()
	...
	self.user.pityManager:incrementPetCount()
	...
	if not self.user.tutManager.completedTutMods["CompleteFirstWave"] then
		petClass = "CappuccinoAssassino"
	elseif not self.user.tutManager.completedTutMods["CompleteSecondWave"] then
	...
	local chosenTutMod = self.user.tutManager.chosenTutMod
	...
	if Common.listContains({
		...
		self.user.notifyManager:notifySuccess(string.format("A %s brainrot has spawned!", rating))
	end
...
function SaveManager:initWaveMod(petData)
	...
	local waveMod = {
		...
		plotName = self.user.plotManager.plotName,
	...
	routine(function()
		for _, waveData in pairs(totalWaveData) do
			...
			for i = 1, count do
				...
				wait(spawnTimer / self.user.speedManager:getSpeed())
			end
		end
	end)
end
...
function SaveManager:addWaveUnit(waveMod, unitClass)
	local unitManager = self.user.unitManager
	unitManager:addUnit({
		unitClass = unitClass,
		waveMod = waveMod,
	})
...
function SaveManager:tryTogglePlay(data)
	...
	if self.tryTogglePlayExpiree and self.tryTogglePlayExpiree > ServerMod.step then
		self.user.notifyManager:notifyError("Clicking too fast")
		return
	end
	self.tryTogglePlayExpiree = ServerMod.step + 60 * 0.5

	-- if not completed tutorial, don't allow to play
	if not self.user.tutManager.completedTutMods["EquipBat1"] then
		self.user.notifyManager:notifyError("Cannot do this yet")
		return
	end
	if self.playing then
		if not self.user.tutManager.completedTutMods["CompleteTutorial"] then
			self.user.notifyManager:notifyError("Cannot do this yet")
			return
		end
	end
...
function SaveManager:completeWaveMod(waveMod)
	...
	self.user.indexManager:unlockPet(petData["petClass"], petData["mutationClass"])
	...
	local autoSellManager = self.user.autoSellManager
	...
	successTimer = successTimer / self.user.speedManager:getSpeed()
