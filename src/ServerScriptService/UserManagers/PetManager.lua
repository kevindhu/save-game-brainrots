...
local PetSpot = require(game.ServerScriptService.Objects.PetSpot)

local PetManager = {}
PetManager.__index = PetManager

function PetManager.new(user, data)
	local u = {}
	u.user = user
	u.data = data
...
function PetManager:init()
	...
	routine(function()
		wait(0.5)
		if self.isNew then
			local plotName = self.user.plotManager.plotName
...
function PetManager:getRandomFrame()
	local plotManager = self.user.plotManager
...
function PetManager:trySwapPetAtPetSpot(data)
	...
	self.user.toolManager:tryPlacePetAtPetSpot(data)
end
...
function PetManager:trySwapRelicAtPetSpot(data)
	...
	self.user.toolManager:tryPlaceRelicAtPetSpot(data)
end
...
function PetManager:tryEquipBestPets(data)
	...
	for i = 1, PET_SPOT_COUNT do
		local petSpotName = self.user.plotManager.plotName .. "_PetSpot" .. i
		local petSpot = self.petSpots[petSpotName]
