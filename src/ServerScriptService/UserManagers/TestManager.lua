...
function TestManager.new(user, data)
	local u = {}
	u.user = user
	u.data = data
...
function TestManager:tryBuyProduct(productId)
	...
	if not productInfo then
		self.user.notifyManager:notifyError("!! PRODUCT NOT FOUND: " .. productId)
		return
	end
	...
	if self.robuxCount < robuxPrice then
		self.user.notifyManager:notifyError("Not enough robux to purchase")
		return
	end
	...
	local shopManager = self.user.shopManager
...
function TestManager:tryBuyGamepass(gamepassClass)
	...
	if not productInfo then
		self.user.notifyManager:notifyError("!! GAMEPASS NOT FOUND: " .. gamepassClass)
		return
	end
