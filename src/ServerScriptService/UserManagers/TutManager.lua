...
local TutInfo = require(game.ReplicatedStorage.Data.TutInfo)
...
function TutManager.new(user, data)
	local u = {}
	u.user = user
	u.data = data
	...
	routine(function()
		...
		if self.isNew or not self.completedTutMods["CompleteSecondWave"] then
			...
			if not Common.isStudio then
				-- clear all petspots
				local petManager = self.user.petManager
...
function TutManager:initFirstTutMods()
	...
	self.user.analyticsManager:logOnboardingFunnelEvent(1, "Tutorial Started", {})
...
function TutManager:chooseTutMod(tutMod)
	...
	if tutName == "CompleteTutorial" then
		self.user.badgeManager:addBadge("CompleteTutorial")
	elseif tutName == "Buy2xSpeedCommon" then
		self.user.badgeManager:addBadge("BoughtTimeFromTimeWizard")
	end
end
...
function TutManager:completeTutMod(tutName)
	...
	self.user.analyticsManager:logOnboardingFunnelEvent(funnelStep, funnelName, {})
	...
	routine(function()
		local favoriteManager = self.user.favoriteManager
	...
	if tutName == "CloseTimeWizard" then
		self.user.speedManager:tryToggleSpeedMod({
			rating = "Common",
			speedIndex = 1,
		})
	end
	if tutName == "Choose2xSpeedCommon" then
		local saveManager = self.user.saveManager
