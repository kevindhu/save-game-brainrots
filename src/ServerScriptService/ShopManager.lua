...
function ShopManager.new(owner, data)
	local u = {}
	u.owner = owner
	u.data = data
...
function ShopManager:rewardProductFromReceipt(receiptData)
	...
	routine(function()
		...
		self.user.home.statManager:incrementStatMod("RobuxSpent", robuxCost)

		print("LOGGING PURCHASE: ", robuxCost, productClass)

		self.user.home.analyticsManager:logCustomEvent("Purchases", robuxCost, {
			"Product",
		})
	end)
...
function ShopManager:tryResolveGiftUserMods()
	for receiptName, giftUserMod in pairs(self.giftUserMods) do
		...
		local otherUser = ServerMod:getUserFromUserId(userId)
		...
		otherUser:notifySuccess("%s gifted you '%s'!", self.user.name, productStats["alias"])

		local otherRewardManager = otherUser.home.rewardManager
...
function ShopManager:tryBuyGamepass(data)
	...
	if self.gamepassMods[gamepassClass] then
		self.user:notifyError("You already have this gamepass!")
		return
	end
	...
	local testManager = self.user.home.testManager
...
function ShopManager:tryBuyProduct(data)
	...
	if not valid then
		self.user:notifyError(string.format("Cannot buy product: %s", reasonText))
		return
	end
...
function ShopManager:addPremiumBenefits()
	-- self.user.home.afkManager:startTimer()
end
...
function ShopManager:tryBuyPremium(data)
	...
	if player.MembershipType == Enum.MembershipType.Premium then
		self.user:notifyError("You already have Premium!")
		return
	end
...
function ShopManager:checkValidProduct(productClass)
	...
	if rewards["permanentToolClass"] then
		local toolMod = self.user.home.toolManager.permanentToolMods[rewards["permanentToolClass"]]
		if toolMod then
